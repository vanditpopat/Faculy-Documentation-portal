...
$query = "SELECT * from any_other_activity inner join facultydetails on any_other_activity.Fac_ID = facultydetails.Fac_ID ";
    ...
    <table  class="table table-stripped table-bordered " id = 'example1'>
        ...
        if(mysqli_num_rows($result)>0  ){
			...
			if($_SESSION['username'] == 'hodextc@somaiya.edu' || $_SESSION['username'] == 'hodcomp@somaiya.edu' )
                ...
                echo "<td>
                    <form action = '3_edit_hod_anyother.php' method = 'POST'>
                        <input type = 'hidden' name = 'id' value = '".$row['any_other_ID']."'>
                        <button type = 'submit' class = 'btn btn-primary btn-sm'>
        ...
        ?>
			...
			<?php if ($_SESSION['rows'] > 0) {?>
	            ...
	            <a href="count_all_anyother.php" type="button" class="btn btn-success btn-sm"><span class="glyphicon ">Count Any Other Activities</span></a> 
	            <a href="export_to_excel_hod_anyother.php" type="button" class="btn btn-success btn-sm"><span class="glyphicon ">Export to Excel</span></a>
